...
Производство физических продуктов: **Torross** (кожа, верёвки, аксессуары).
...
- **Admin: восстановление удалённых комментариев/сообщений** (2026-09-22): `PATCH`-роуты на `/api/admin/comments/[id]` и `/api/admin/messages/[id]` получили Restore (снимает `isDeleted`/`deletedAt`/`deletedById`, пишет `ModerationAction` `*.restored`); в `AdminConsole`/`ReportDetail` разрешённый-но-удалённый репорт остаётся достижимым для восстановления вместо того, чтобы просто пропасть из списка
- **Явное серверное удаление EXIF/GPS** (2026-09-22, `lib/utils/stripExif.ts`): раньше метаданные снимались только побочным эффектом клиентской canvas-компрессии — теперь снимаются всегда на сервере, независимо от клиента. JPEG (Exif + Adobe XMP в APP1), PNG (`eXIf` + XMP в `iTXt`), WEBP (EXIF/XMP RIFF-подчанки, с пересчётом размера контейнера) — без переэкодирования, побайтовым удалением сегментов/чанков. GIF сознательно не трогается (нет стандартного носителя GPS). Проверено синтетическими тестовыми изображениями с GPS и в EXIF, и в XMP
- **Приватное хранилище фото** (2026-09-22, DECISIONS.md): бакеты `post-photos` и `avatars` переведены из public в private; чтение теперь только через короткоживущие (15 мин) signed URL, минтящиеся server-side (`lib/storage/signed-url.ts`, `lib/storage/resolve-media.ts`) — подключено во все ~21 точки чтения (лента, посты, комментарии, профиль, hall, members, rooms, DM). Бэкфилл существующих `mediaUrls`/`avatarUrl` в голые пути — с полным бэкапом оригиналов в `MediaUrlBackfillBackup` (обратимо). Явные deny-select policy на `storage.objects` как defense-in-depth. Проверено вживую на проде: старая публичная ссылка больше не работает, signed URL работает, разлогиненный/удалённый участник новых ссылок не получает
- **Admin: Accept/Hold/Decline для email captures** (2026-09-22): раньше "Email captures" на `/admin` были просто списком без действий. Теперь каждая строка получает Accept/Hold/Decline, которые лениво создают/переиспользуют строку `Waitlist` (origin `email_capture`) и вызывают ту же логику approve/hold/decline/resend, что и обычные заявки (`lib/admin/waitlist-decisions.ts`) — один общий пайплайн, без дублирования. Если Resend не настроен — состояние всё равно меняется, письмо помечается как непроверенное и попадает в существующий счётчик "Failed Sends"

### 🔄 СЛЕДУЮЩИЙ ПРИОРИТЕТ
1. **Desktop DM thread layout** — на десктопе composer уходит ниже линии видимости при открытии треда; нужен full-height flex-layout (шапка/composer закреплены, скроллится только список сообщений), по образцу уже рабочей мобильной вёрстки. Задача поставлена, ещё не начата
2. **Apple OAuth (реальные креды)** — Apple Developer аккаунт, Services ID, ключ → настроить в Supabase Dashboard → включить флаг
3. **Остальные дома** — названия и категории для оставшихся 8, когда решит Max
4. **Реальные предметы в Vault** — Max создаёт их через `POST /api/admin/vault-items`, когда определится каталог
5. **MarketplaceItem vs VaultItem** — решить, нужна ли отдельная модель для маркетплейса мерча/билетов, или Vault её заменяет тоже
...
*CLAUDE.md v2.1 — обновлён 22 сентября 2026*
