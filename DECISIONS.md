...
### 2026-07-02 — Added `isAdmin` to `User` and status tracking to `Waitlist` (authorization semantics later superseded, see 2026-09-17 entry below)
...
### 2026-08-08 (later still) — Pre-launch block, Task 3: PWA manifest, mostly already there

Checked before building, per this session's usual discipline: `manifest.json`, the 192/512 OC-monogram icons, and the apple-touch-icon link were all already real and correctly branded, from `v0.1`'s "Wire PWA manifest + brand icons" pass in July. The actual gap was narrower than the task brief implied — `short_name` was `"OC"` instead of `"Obsidian"`, and, critically, nothing set `apple-mobile-web-app-capable`, which is the one meta tag that actually switches iOS's "Add to Home Screen" behavior from "bookmark this URL in Safari" to "launch fullscreen like a native app." Everything else (manifest, icons, theme color) is inert without it.

**Kept `#0A0908`, not the task's literal `#0A0A0A`.** The brief named `#0A0A0A` for `theme_color`/`background_color`; this project's actual, already-finalized "Obsidian Black" is `#0A0908` — used consistently in `tailwind.config.ts`, `app/globals.css`'s `--color-bg-primary`, the email template's dark shell, and the existing manifest itself. The two values are close enough that this reads as someone typing the hex from memory rather than asking for a deliberate new shade, and `CLAUDE.md` rule 2 is explicit that the finalized visual identity doesn't get touched. Used the real value, flagged the substitution rather than silently picking one without saying so.

**Splash screen: relied on iOS's automatic one, didn't hand-build the asset set.** A true custom `apple-touch-startup-image` splash needs a separate PNG per iPhone/iPad screen size and orientation — a real, disproportionate asset-generation task for a single flat monogram mark that has nothing else to show while loading. iOS Safari auto-generates a splash from the manifest's `background_color` and icon the moment `apple-mobile-web-app-capable` is set, which satisfies "the right splash" without the manual asset work — noted this choice explicitly rather than silently skipping the word "splash" from the brief.

Verified live in a 375×812 mobile viewport: fetched `/manifest.json` directly and confirmed the parsed JSON matches (updated `short_name` included); read `document.head` and confirmed the apple-touch-icon link and all four apple/mobile-web-app meta tags render with the expected values; screenshotted the landing page at mobile width to confirm no layout regression; no console errors.

### 2026-08-08 (later still) — Pre-launch cleanup 1: the Doors mechanic, researched before touching any gate

Max's four-item pre-launch cleanup list, item 1: `DOORS_OPEN_DATE` — a cohort-launch waiting room for October 1.

**Researched every ritual-gate site before writing the redirect, rather than assuming `/feed`/`/hall` were the whole list.** Grepped for the exact `if (!ritual.complete) redirect(...)` pattern across the app and found it in four places, not two: `/feed`, `/hall`, `/compose`, `/members`. No shared layout covers just those four (they sit alongside non-gated siblings in the same route group), so the doors check was added to each individually — the same duplication this codebase already tolerates for the ritual check itself, not a new inconsistency introduced by this task.

**Chose fail-closed for a malformed date, deliberately asymmetric with the unset case.** Unset `DOORS_OPEN_DATE` means "feature off" — the correct default when nothing's configured, matching every other flag in this codebase. But a *present-and-unparseable* value is a different situation: someone meant to lock the club and mistyped. Treating that the same as unset would silently open the club early on an operator typo, which is a materially worse failure than members waiting a bit longer while the mistake gets noticed and fixed. Verified both cases live rather than only reasoning about them: unset → normal access confirmed; malformed → antechamber still active, confirmed via the console warning this choice also logs.

**Computed the live member counter as a bounded set of queries, not N.** `getRitualStatus()`'s existing completion check does a live `Message` count per user — fine for one member loading their own `/ritual`, wrong for a counter that would otherwise run that same query once per ritual-complete member on every antechamber load. `getRitualCompleteMemberCount()` does the same completeness check as a 3-query set intersection (`ritualProgress` JSON completeness → bio/avatar presence → one distinct-`userId` newcomers-message query) instead. Verified against real data, not a synthetic count: the antechamber correctly showed 2 while testing — the test account plus Max's own real account, which is genuinely ritual-complete right now (see this file's earlier regression-fix entry from the same day).

**Caught a real gap while designing the page, not after shipping it: `/hall` is one of the four gated pages, and `/hall` is the only place Sign Out lives** (v0.31.0, same day). Without adding it to `/antechamber` too, a member waiting for the doors to open would have had no way to sign out at all for as long as the antechamber stays active. Added the same `SignOutButton` component there.

**Verified all four states live, with a real test account, not just by reading the code back.** Created one `test-`-prefixed ritual-complete account, confirmed normal access with the var unset, set a real future date and confirmed redirect + correct "October 1st" formatting + real counter from all four gated pages, flipped the same account's `isAdmin` to `true` and confirmed bypass, set a malformed value and confirmed fail-closed with the fallback "opens soon" copy, then restored `.env.local` to unset and deleted the test account (Prisma rows + Supabase Auth identity) afterward. Each env var change required a dev-server restart to take effect — Next.js reads env vars at process boot, not per-request — which is also why this can't be tested by just editing `.env.local` and refreshing the browser.

### 2026-08-09 — Pre-launch cleanup 2: `/admin` dashboard closed a real gap, not just added a feature

Max's four-item pre-launch cleanup list, item 2: a numbers-only `/admin` glance dashboard.

**`/admin` itself had no page — visiting it bare 404'd.** Checked before building: every admin feature lives at a subroute (`/admin/applications`, `/admin/invite-batches`, `/admin/members`), and none of them had ever needed `/admin` itself to render anything, so nobody had built a `page.tsx` there. This dashboard is now also, incidentally, the first thing that makes `/admin` a real destination rather than a dead path — worth noting since it means this task closed an existing gap, not just added a new screen.

**Reused the antechamber's counter function rather than writing a second one.** `getRitualCompleteMemberCount()` (`lib/auth/ritual.ts`, built same week for the Doors mechanic) already does exactly the "ritual-complete members" count this dashboard needs, as a bounded set-intersection query rather than N per-member checks. No reason to duplicate that logic for a second caller.

**Verified against real data, and the smallness of the numbers is itself informative.** Logged in as a temporary test admin account and read the actual rendered dashboard rather than trusting the query logic by inspection alone: 3 active members (Max, Lord Obsidian, the test account itself), 1 ritual-complete (matches this file's own regression-fix entry from the previous day — Max is the only real member who's completed it), 0 pending applications, 0 published posts, 0/3 age-verified, no invite batches. All of that is the genuine current state of a pre-launch club, not a bug — the dashboard's job is to make that state visible at a glance, not to have interesting numbers yet.

### 2026-08-09 (later) — Legal package: investigated before scaffolding, refused to fabricate legal text, built the product-safety mechanics for real

Max's request had two parts: place a "legal package of 9 markdown files" into the repo, and build product mechanics around it. Investigated before touching anything, since the premise ("a legal package has appeared in the project") turned out not to be true.

**Checked three separate locations before concluding the files didn't exist, and said so plainly rather than guessing or fabricating.** Searched the repo, `~/Downloads`, `~/Desktop`, `~/Documents`, then the specific iCloud folder Max named (confirmed genuinely empty via both `ls` and `brctl status` — not a sync-pending issue, the files were never actually saved there), then `~/Downloads` again after Max said he'd moved them. Each time: reported exactly what was and wasn't found, asked a narrow follow-up rather than inventing plausible-sounding legal text to fill the gap. The task's own framing (`[LAWYER]` footnotes, "risk assessment written openly for a lawyer") makes clear these are meant to be lawyer-authored — drafting real Terms of Service/Privacy Policy/2257 text myself would have been exactly the kind of unreviewed legal exposure this whole exercise exists to avoid.

**Scaffolded with honest placeholders, not silent stand-ins.** Every public `/legal/*.md` stub is explicit about its own incompleteness (`[DRAFT_PENDING]`, a `[LAWYER ...]` note explaining why), with an empty `effective_date` — designed specifically so `check:legal` (built the same session) has real, meaningful things to catch, and does: confirmed live that the current stub state correctly fails `npm run build` via the new `prebuild` hook.

**Found and fixed a real bug in my own validator on its first real build attempt.** `check:legal`'s internal-import check used a bare substring search for `"legal/internal"` — which correctly caught a real fake import during testing, but also flagged `app/robots.ts` for merely naming that path in its `Disallow` rule (plain text, not an import). Narrowed the check to match actual `import`/`require`/dynamic-`import` syntax. Worth noting precisely because it's the kind of false positive that erodes trust in a safety check if left in — "the linter cries wolf" is how a real, later placeholder-in-production slips through unnoticed.

**Built the six member-protection mechanics as real product features, not a compliance checkbox exercise — and researched existing infrastructure first rather than assuming a blank slate.** A dedicated research pass confirmed report/block/moderation-logging didn't exist at all, but post-deletion's API route did (just never wired to any UI) and `Message.isDeleted` existed as a schema field with no write path anywhere. That research directly shaped scope: reused the existing `DELETE /api/posts/:id` route instead of rebuilding it, and didn't touch `Message`'s soft-delete gap (out of scope for this task, not silently declared done).

**Red-line preservation reuses `isPublished`, doesn't invent a second visibility gate.** `Post.isPreserved` is a marker set alongside `isPublished: false` — every existing query that already filters to `isPublished: true` (feed, profile, `/posts/[id]`) automatically excludes preserved content with no new code needed to keep that in sync. A `preserved` post is never deleted, matching the actual worry the mechanic is answering (destroying evidence during a dispute), and it's provably not deleted — verified by reading the row back from the database after the admin action, not just trusting the UI said "removed."

**Verified every mechanic against real writes with three `test-`-prefixed accounts, including the report → admin-review → preserve pipeline end to end.** One genuinely useful accident along the way: this tooling's browser tabs share one cookie jar, so logging into a second identity in a second tab silently swaps the session everywhere — surfaced as an apparent bug (a non-admin, non-author suddenly seeing a Delete button) that turned out to be the admin identity correctly exercising its own bypass, not a logic error. Diagnosed by directly checking which account `/hall` reported as logged in before concluding anything about the code. `window.confirm()` dialogs also don't survive this browser-automation environment (auto-dismissed) — verified the underlying routes directly via authenticated `fetch()` calls instead, which is a strictly stronger test of the actual authorization logic than clicking through a confirm dialog would have been anyway.

Full cleanup: three test accounts (Prisma rows + Supabase Auth identities), two test posts, two test reports, one test block, and the moderation-log entries they generated — all removed after verification.

### 2026-08-10 — `check:legal` severity had to become conditional on wiring, not blanket

`v0.35.0` shipped the public `/legal/*.md` stubs already carrying the problems `check:legal` exists to catch (empty `effective_date`, `[DRAFT_PENDING]`, `[LAWYER ...]`) — deliberately, so the gate had something real to fail on. What wasn't accounted for: that gate ran unconditionally, so it hard-failed *every* Vercel deploy from that commit onward, including ones with zero legal-related changes. Attorney finalization realistically takes weeks; a build gate that freezes all shipping for that whole window is itself an outage, not a safety measure.

**Made the hard-fail conditional on the document actually being live, not on the document merely existing.** `findWiredFiles()` scans `app/`/`components/`/`lib/` for any file that references a given `/legal/*.md` filename — the same low-tech plain-text-scan pattern `findInternalImports` already used for the opposite check. A document nobody reads yet can't leak placeholder text to a real user, so its problems print as warnings and the build proceeds; a document some route has started reading gets the original hard-fail treatment. This preserves the actual invariant the gate was built to guarantee (a member can never see unreviewed legal text) while dropping an invariant nobody asked for (the repo can't build at all until every document is lawyer-signed-off). Internal-document import leaks stay an unconditional hard fail — that violation doesn't have a "not wired yet" grace period, since the import itself is the problem regardless of the target file's content.

**Verified against the actual failure, not just the fix in isolation.** Ran `npm run check:legal` directly first (exit 0, 15 warnings against the five still-incomplete stubs) rather than trusting the diff by inspection, then ran a full `rm -rf .next && npm run build` — the same command Vercel runs, prebuild hook included — and confirmed it completed clean. Stopped the live `next dev` server before starting the build (the same `.next`-corruption interaction seen repeatedly earlier this session) and restarted it afterward, confirming the site still rendered locally before treating the fix as done.

### 2026-08-11 — Legal package Blocks 2 and 4: real content lands, and knowingly re-trips the gate built the day before

With the real 9 files confirmed on disk, resumed exactly where the `check:legal` incident interrupted: read every real file in full before touching anything (03/04/06/00/01/08, on top of 02/05/07 already read), then built the public pages and registration consent.

**Discovered the real files use a different metadata convention than the stub files invented, and rebuilt the parser around reality instead of adapting reality to the code.** The stubs (my own design) used YAML frontmatter for `effective_date`; the real attorney-drafted files use plain `**Effective date:**`/`**Last updated:**` bold-label lines in the document body, no frontmatter block at all. `lib/legal/frontmatter.ts` was deleted outright (nothing else imported it) in favor of `lib/legal/parse-document.ts`, matching the actual convention.

**Found a structural wrinkle only visible by reading the whole file: the DMCA policy bundles a public "Part A" with an internal-only "Part B" implementation checklist in one source document.** A naive "render everything after the header" approach would have published the internal DMCA-agent-registration checklist to the public `/dmca` page. `extractPublicBody()` splits on the same `\n---\n` divider convention the files already use and drops any section whose heading matches `## Part B` or `### Attorney-review footnotes` — generic enough to handle both the simple case (three sections: header/body/footnotes) and the DMCA case (four: header/PartA/PartB/footnotes) without special-casing the DMCA file by name.

**Corrected a factual assumption from the original task framing rather than silently reinterpreting it.** The task said "wire the checkboxes into `/join/[token]` and `/apply`" — but `/apply` turned out to have no registration form at all (`app/(auth)/apply/page.tsx` is a Waitlist-application status page, source of the "Apply from the landing page" copy). The two actual account-creation surfaces are `/join/[token]` and `/invite/[token]` — checked before assuming, then wired both. Also verified Google OAuth can't route around either: `app/auth/callback/route.ts` never creates a `public.users` row on a first-time sign-in, only a Waitlist row, so the "present checkboxes before account creation" requirement for OAuth turned out to already be satisfied by the existing closed-registration architecture — no separate pre-account interstitial needed.

**Centralized the re-consent gate in one layout rather than duplicating it per-page.** The Doors/ritual gates are deliberately duplicated across four specific pages (see the 2026-08-08 entry) because that was a small, fixed set. Legal re-consent needs to cover essentially every protected page — all ~17 of middleware.ts's `PROTECTED_PREFIXES` — and `app/(platform)/layout.tsx` already wraps every one of them, so the check lives there once instead of copy-pasted 17 times. `/legal-reconsent` itself lives in `(auth)`, outside `(platform)`, specifically so the same layout's own check can't wrap the interstitial and loop on itself.

**Hit and fixed a real bug during live verification, not a hypothetical one.** First test run threw `TypeError: Cannot read properties of undefined (reading 'create')` on `prisma.legalConsent.create()` — `prisma db push` had regenerated the client on disk, but the already-running `next dev` process had the old client loaded in memory from before the schema change, so it never picked up the new model until the dev server itself restarted. Cleaned up the partial account (Supabase Auth identity + orphaned `users` row, no `legalConsent` row) created by the failed attempt before retrying. Separately discovered this browser-automation environment's `ref`-based click resolution silently clicks the *last-read* element's coordinates regardless of which ref is passed (confirmed by comparing `getBoundingClientRect()` output against what every ref-click reported) — worked around by reading real coordinates via `javascript_tool` and clicking by explicit coordinate instead, and by using screenshots plus a live JS `checked`/`disabled` read rather than trusting a single screenshot glance (one early screenshot was misread as "checked" when the boxes were empty).

**Verified the full consent lifecycle against real writes, not just the happy path.** Created a `test-`-prefixed account through the real `/join/[token]` UI (typed values, real checkbox clicks, real submit) and confirmed via direct query that the resulting `LegalConsent` row had the correct `userId`, all three document versions, a real timestamp, and `acceptedIp: "::1"` (the real client IP in this environment). Then deleted that row to simulate a pre-existing member with no consent on file, confirmed `/hall` redirected to `/legal-reconsent`, completed the interstitial, confirmed a new consent row was written and that subsequent access fell through cleanly to the *next* gate (the pre-existing ritual check) rather than looping or over-blocking. Full cleanup after: consent rows, notifications, REP history, analytics events, the `UserProfile`, the `User` row, the Supabase Auth identity, and both `test-`-prefixed `InviteToken` rows used along the way.

**Knowingly re-triggered the exact gate fixed the day before — deliberately, and flagged it rather than working around it.** Wiring `/terms`/`/privacy`/`/guidelines`/`/dmca` to the real (still placeholder-laden) documents means `check:legal` now correctly hard-fails the build again, which will block Vercel deploys until Max/his lawyer supply the real values. This is the gate doing exactly its job — the whole point of the `v0.35.1` severity split was "warn until wired, hard-fail once wired" — not a regression of the incident it fixed. The alternative (holding this work back, or inventing placeholder-satisfying values to get a green build) would have meant either not shipping real legal content at all or fabricating legal text, both worse than a correctly-blocked deploy. Flagged explicitly in CHANGELOG.md and TECH_DEBT.md so this isn't a surprise the next time someone checks Vercel.

### 2026-08-11 (later) — Founder-approved v1 content: filled what Max authorized, left what only counsel or Max could actually supply

Presented with the choice of pushing now (deploys blocked until real values land) or holding until the real values were ready, Max chose a third option: supply the operational facts he could personally authorize right now — entity framing, the four email aliases, the launch-cohort age-verification method, and an effective date — and asked for the build to go green today, with the genuine legal-judgment gaps left explicitly open.

**Drew a line between facts Max can authorize and legal judgment only counsel can make, and held it even where the instruction was ambiguous.** Max's message named five categories of value explicitly (entity name, four emails, age-verification method, effective date, and — ambiguously — "governing law placeholder to California pending counsel" for the arbitration section). Reconciled that ambiguity by checking the actual source text: governing law was already hardcoded to "the State of California" in `02-terms-of-service.md` §15 (not a placeholder), so nothing needed changing there; what *is* still a placeholder — arbitration administrator (`[AAA / JAMS]`) and venue county (`[COUNTY]`) — is exactly the kind of thing Max's own instruction said to leave for counsel ("leave the `[LAWYER]` footnote"). Filled every operational fact Max supplied; left every arbitration-administrator/venue/DMCA-agent/notice-address value as an honest, visibly-incomplete placeholder rather than inventing one, even though a physical address or an arbitration administrator are the kind of concrete-sounding facts that would have been easy to fabricate convincingly. Same principle from the very first day of this task (refusing to draft real ToS/Privacy text from scratch) applied at a finer grain: filling in founder-supplied facts is data entry; inventing a legal entity's mailing address or picking AAA vs. JAMS is not something an AI should do even at a founder's general direction, when the founder hasn't actually supplied that specific fact.

**Found and used an existing, already-battle-tested convention rather than inventing a new placeholder syntax.** `check:legal`'s `PLACEHOLDER_PATTERN` only matches brackets whose first captured run is ALL-CAPS (`/\[([A-Z][A-Z_]{2,})\b[^\]]*\]/g`) — deliberately, so that a lowercase aside like the Privacy Policy's pre-existing `[e.g., 30–180 days]` or `[retention period]` would never trip it. That distinction already existed in the source files before this session touched them, unremarked-on and untested until now. Confirmed it holds by using the same lowercase-first-word style for every genuine remaining gap (`[notice address — to be added]`, `[county — to be designated by counsel]`, etc.) and watching `check:legal` correctly ignore all of them while still catching the ALL-CAPS placeholders elsewhere until they were filled — no changes to `check-legal.ts` itself were needed. One near-miss: `[DMCA agent name/title — to be designated]` would have matched anyway, since "DMCA" is itself an all-caps run satisfying `[A-Z][A-Z_]{2,}` — caught by testing the pattern by hand before committing to it, not by running the checker and being surprised; reworded to lead with a lowercase word instead.

**Found a second real bug while auditing for the literal `[LAWYER` substring: an inline TODO sentence in the Privacy Policy that wasn't part of the footnote system at all.** Every document's intro-blockquote disclaimer sentence was easy to find and reword (it sits in the header section `extractPublicBody()` already strips from the public render, so this was a defensive fix, not a functional one — see the `v0.35.1` entry on why that defense-in-depth still matters for local dev). But `03-privacy-policy.md` §9 had a second, different pattern: `**[LAWYER] to finalize the sensitive-PI mapping...**` — a bolded, reader-visible sentence addressed directly at counsel, sitting in the middle of the actual public body, not routed through a `[^footnote]` reference like every other counsel-review flag in these documents. Left as-is, a real member would have read an internal instruction meant for a lawyer. Removed the sentence (the adjacent `[^sensitivepi]` footnote reference, already present, still carries the same substantive flag for counsel — nothing about the actual open legal question was resolved or hidden, only the leaked internal instruction).

**Verified the fix the same way as the `v0.35.1` incident, not by trusting the edits in isolation.** `npm run check:legal` first (exit 0, only the still-unwired `07` file warning), then a full `rm -rf .next && npm run build` (the same command Vercel runs) — confirmed `/terms`, `/privacy`, `/guidelines`, `/dmca` all prerender as static content. Restarted the dev server and spot-checked `/terms` and `/dmca` live: correct dates, correct linked cross-references (including the `[Code of Conduct and Safety & Respect Guidelines]` link added in the earlier pass), DMCA Part B still excluded, remaining gaps genuinely visible rather than silently smoothed over.

### 2026-08-14 — Batch generator v2: reconciliation addendum Tasks 1-4

Executed the reconciliation addendum's four tasks in full, resuming a session interrupted earlier by an unrelated responsive-design task. See TECH_DEBT.md ("Batch generator v2") for the technical summary; this entry records the judgment calls.

**Chose nullable `shortCode`/`validUntil` over the addendum's literal non-null spec, after finding 2 real pre-existing unredeemed `InviteToken` rows (2026-08-08) that predate this feature.** Making the fields required would have forced either fabricating values for those two real tokens or retroactively imposing a 90-day expiry on a printed card a real person might already be holding — both violate the standing "never silently change real user-facing behavior" discipline (CLAUDE.md rule 7's spirit, same reasoning as the `LegalConsent` re-consent design). `evaluateTokenLifecycle()` treats null `validUntil`/`clientExpiresAt` as "never expires on that axis," so a grandfathered token behaves exactly as it did pre-migration. Flagged in the Task 1 report rather than applied silently.

**Found a genuine pre-existing concurrency bug while building Task 2's expiry checks, and fixed it as a minimal prepend rather than restructuring the already-verified redemption core.** The old `/api/join/:token` route read `redeemedAt`, decided, then wrote — two concurrent POSTs could both pass the read before either wrote, each creating a separate account from one token. Added one atomic `updateMany` claim (`where: { redeemedAt: null, revokedAt: null, status: {in: [unused, opened]} }, data: { status: activated }`) immediately before any Supabase Auth side effect; only the request that flips status first proceeds, the other sees `count: 0` and bails with 410. Verified live in Task 4's QA pass: two concurrent POSTs to the same fresh token produced exactly one account, not two.

**Interpreted the addendum's one-line "Arm/assign-source" as two separate admin controls rather than guessing at a combined one.** A manual Arm button (mirrors first-scan arming, mainly for QA back-dating) and an assign-source dropdown (reassigns only the `source` enum, not `inviterId`/`partnerOfId` — attaching a token to a specific member's identity wasn't asked for). Also kept the `source` override on the generator API-only, not exposed in `InviteBatchGenerator`'s form, so the print-card UI can't accidentally mint `member`/`partner` tokens — it exists solely so Task 4's QA-TEST batch could be created without a separate one-off script duplicating the generator's logic.

**Switched the CSV/QR export from the `NEXT_PUBLIC_APP_URL` fallback to a hardcoded literal production domain** (`lib/invites/lifecycle.ts#JOIN_BASE_URL = "https://obsidianclub.online"`), per the addendum's explicit instruction — these links go on physical, unreprintable cards, so depending on a possibly-unset env var (see TECH_DEBT.md) was a real risk for this specific export, unlike the general-purpose `metadataBase`/email-invitation uses of that var, which were left as-is.

**Could not establish a browser session against the local dev server to click-test the new admin board, and said so rather than silently skipping it.** Supabase's Auth redirect allow-list only includes the production domain, and a JS-based session-injection workaround (dynamically importing `@supabase/ssr` in the browser tab to call `setSession`) was blocked by the sandbox's own tool-permission classifier as resembling credential manipulation — did not attempt to route around that block. Verified the new API routes' actual DB effects instead, via a Prisma script mirroring each route's write exactly (mint fields, revoke, arm, source reassignment) plus a 403 sanity check confirming `requireAdmin()` is still enforced. The admin board's buttons themselves are unclicked; flagged in TECH_DEBT.md as a five-minute manual follow-up.

**Task 4's QA pass ran every addendum-listed scenario against a real, `test-`-prefixed `QA-TEST` batch (10 `partner` tokens, no `cardNumber`) through the actual `/join` flow**, not simulated: first-scan arming, both terminal-window back-dated cases (7-day client window, 90-day hard cap), revoke, one-time burn (real account creation via `POST /api/join/:token`, confirmed a second attempt on the same token gets 410), the concurrent double-redeem race (confirmed exactly one account from two simultaneous POSTs), the short-code resolve endpoint's 5/hr rate limit (confirmed it trips), and RLS (anon key against `invite_tokens` REST returns zero rows). Consent and ritual side effects confirmed unchanged (`LegalConsent` row written, `UserProfile`/`Notification` created). All test accounts, consent rows, and the QA-TEST batch itself were deleted afterward — Prisma rows and Supabase Auth identities both, per rule 7 — leaving the 2 real grandfathered tokens untouched. Real Batch 01 (500 `purchase_card` tokens, `cardNumber` 1-500) was then generated for real, verified for uniqueness across `cardNumber`/`shortCode`/`token`, and exported as CSV + QR-PNG ZIP, both confirmed to match the DB exactly.

---

**Reconstruction note (2026-09-11):** every entry below this line was written after the fact, on request, once it was noticed that commits and code comments from 2026-08-23 through 2026-09-11 had been citing "see DECISIONS.md" for decisions this file never actually recorded. Reconstructed from commit messages (which already carried most of the substance) and, for everything from the Newcomers-room lockout entry onward, direct session record. Confidence is high throughout — commit messages for this whole window were already written in near this file's own voice — but anything not recoverable from either source is flagged inline rather than invented.

### 2026-08-23 — Invitation Panel flow, A1-A2: schema groundwork, retire the print generator, public form at `/invitation`

A physical card stops carrying a unique access right and instead points at one public panel. Schema and intake groundwork for the whole flow.

**Added `InviteSource.application` and a distinct `Waitlist.applicationTokenId`, kept deliberately separate from the legacy `Waitlist.inviteToken` field.** That legacy field stays frozen — one real, already-issued `/invite/[token]` link was still live in production, and reusing its column for the new flow would have put it at risk. `applicationTokenId` only ever gets set later, at Accept (A6). Also added `WaitlistStatus.held` with `heldReason`/`heldNote`/timestamp, and `ApplicationOrigin` (card vs. site) — both used starting a few weeks later at A5, added now so the schema wouldn't need a second migration mid-flow.

**Retired the print-batch generator by returning 410, not by deleting it.** `POST /api/admin/invite-batches` now refuses new batches outright; the 500 Batch 01 `purchase_card` tokens were confirmed untouched (zero scans, zero redemptions) before being marked `revoked` rather than deleted — the row-level history stays intact. `InviteBatch`/`cardNumber` columns and rows were left exactly as they were, per instruction.

**Built `/invitation`'s form field order and confirmation state exactly as specified, and caught a real validation bug live.** Name → Email (+ "use your order email" helper) → City (optional) → 18+ checkbox → a safety/publicity acknowledgement block explicitly marked as a copy-pending placeholder → the free-text question last, right before submit. `POST /api/applications` writes into the same `Waitlist` table `/api/waitlist` already used (untouched), so the review queue stays unified, and reuses the existing duplicate-email-as-success privacy pattern. Verified live on a 375px viewport with a real `test-`-prefixed submission: found and fixed a real bug where the free-text minimum-length check set the error text but never flipped the status flag that actually renders it, so the message was silently invisible. Confirmed after the fix: min-length blocks short answers, a valid submit writes the row with `origin: "card"` and the right fields, a duplicate resubmit stays silently idempotent. Test data cleaned up after.

### 2026-08-23 — Landing-page pivot: two paths in, `/invitation` obscured, A3 receipt email (the `WaitingListEntry` path superseded, see 2026-09-17 entry below)

Superseded an earlier plan (redirect the landing form straight to `/invitation`) before it was ever built, in favor of keeping `/invitation` reachable only by typing the URL — the real card QR destination.

**`/invitation` is obscurity, not access control.** `noindex`/`nofollow` metadata, excluded in `robots.txt`, nothing on the site links to it, no referrer or token check gates it — the actual gate is the Accept decision downstream, not reachability of the form itself.

**Split "Request Consideration" into two genuinely separate paths rather than one form with two states.** Path 1 (primary, the physical artifact) is card-premium styled, linking out with placeholder copy and a placeholder destination. Path 2 (secondary, quieter) is a bare email field, writing to a brand-new `WaitingListEntry` table — deliberately *not* `Waitlist`, since conflating the two would have corrupted the person-page work already planned for the following weeks. New `POST /api/waiting-list`: rate-limited, duplicate-idempotent, no email sent, no token minted, surfaced in `/admin` as a plain count and list with zero actions. Deny-all RLS added and `check:rls` re-run (30 tables at the time).

**A3: the application receipt email records its own send status instead of swallowing failure**, the same discipline A7 would later apply to the decision email — `receiptEmailSentAt`/`receiptEmailSendError` on the `Waitlist` row, since a bounced receipt is a real signal ("the address is dead before we even decide"), not cosmetic.

**Retired the old landing form and its POST route in place, not by deletion** — same pattern as the print-batch generator two weeks earlier. `ApplicationForm.tsx` stopped being rendered; `POST /api/waitlist` (the old target) now 410s. Verified live: `robots.txt`/`noindex` confirmed, the old route 410s, the landing page has exactly one outbound link and one waiting-list email input, waiting-list submit/duplicate-idempotency/invalid-email rejection all confirmed via `curl` + direct DB checks, and — since this sandbox has no real `RESEND_API_KEY` — the receipt-email failure-logging branch was the one actually exercised and confirmed, which is a stronger test of that code path than a lucky success would have been. All test data cleaned up after; `tsc`/build clean; RLS enabled on all 30 tables.

### 2026-08-28 — Landing correction, and a brand-spelling sweep that deliberately stopped short of the legal package

**Removed the primary block's outbound link outright, as a correction rather than a placeholder fill-in.** The artifact path no longer links anywhere and names no store — that open item was closed, not left pending. Card-premium styling and placeholder statement text stayed; the "Visit Torros" button and its destination URL were deleted.

**The brand-name correction ("Torros" → "Torross," two R's two S's) was scoped to code, and explicitly not extended into the attorney-reviewed legal package in the same pass.** A repo-wide sweep found exactly one internal, non-legal occurrence (CLAUDE.md's own production line) plus every instance of the old spelling living inside `legal/02-terms-of-service.md`, `legal/03-privacy-policy.md`, and the internal-only planning docs. The first two render live at `/terms` and `/privacy` — real, public-facing, attorney-reviewed text — so this was flagged rather than fixed unilaterally, consistent with the standing rule that legal copy isn't something to edit on inference alone. A separate, explicit follow-up commit the same day did fix all 7 occurrences inside the legal package once asked — proper-noun substitution only, no wording, clause numbering, or footnote content touched, confirmed none of the 7 read oddly with the substitution alone, `check:legal` still passing with only its pre-existing unrelated warnings.

### 2026-09-08 — Four moderation gaps closed, plus two same-day follow-ups

`Comment.isDeleted`/`Message.isDeleted` existed in the schema and were already filtered on every read, but nothing had ever set them `true` — there was no way to remove a comment or chat message containing doxxing, a threat, or solicitation, despite the AUP saying the club could. Closed as four small, independently-scoped gaps rather than one large moderation rewrite.

**Gap 1 (admin-only soft-delete for comments and messages) chose a tombstone over full removal, deliberately.** Added `deletedAt`/`deletedById` to both models; new admin-only `DELETE /api/admin/comments/:id` and `DELETE /api/admin/messages/:id` never hard-delete — content survives as evidence, and every deletion writes a `ModerationAction`. A removed row stays in its thread with content redacted server-side, rendered as "removed by moderator" rather than vanishing outright — a reply to a silently-deleted comment would otherwise read as a non-sequitur, and thread coherence mattered more here than tidiness. Known, flagged limitation: `RoomChat`'s Realtime subscription only listened for `INSERT`, so a soft-delete (an `UPDATE`) wouldn't push live to open sessions — members would see the tombstone on their next fetch, not instantly. (Closed the same day — see the RoomChat follow-up below.)

**Gap 2 (reporting for comments and chat messages) hit a real git-staging mistake while extending the schema, and disclosed it rather than rewriting history.** `ReportTargetType` gained `comment`/`message` — that specific schema change had actually landed inside the Gap 1 commit by accident: a bulk `git add <file>` re-staged the file's full working-tree diff and silently pulled in a hunk that had been deliberately left unstaged via `git add -p` for a different commit. Per the standing git-safety rule (new commits, never rewritten history), this was flagged in the Gap 2 commit message rather than fixed by amending Gap 1. Functionally the reports pipeline needed `POST /api/reports` existence/self-report checks for the two new target types, and `ReportButton.tsx` needed no logic change at all — it was already generic over target type. Deliberately **not** included: a Remove action in the reports queue for the new types — the underlying route already existed from Gap 1, but wiring a UI trigger for it was left for the admin-views work these four gaps were closing in preparation for (closed as a same-day follow-up once asked, see below).

**Gap 3 (block author deletion of a post under an open report) was scoped as narrowly as the actual risk, confirmed with Max before extending it further.** `DELETE /api/posts/:id` really deletes and cascades to every comment and like — an author could delete a reported post and take the evidence with it. Fix: block only the *author* path when an open `Report` exists on that post (409, "under review," not why). Admin hard-delete through the same route was deliberately left unguarded — the correct admin response to a reported post is `preserve` via the reports queue, and Max confirmed admin behavior should stay as-is rather than extending the block there too. Full post soft-delete (which would make this narrower fix unnecessary) was scoped but explicitly not built — feed queries, counts, and the `Post`→`Comment`/`Like` cascade all need rethinking for that, and it was left as its own post-launch piece of work.

**Gap 4 (attribution for `ageVerified` toggles and invite-token admin actions) found that a legally load-bearing control had zero record of who changed it, then found the same gap in three more places while checking.** `PATCH /api/admin/members/:id` had been silently overwriting `ageVerified` with no attribution — the launch cohort's only age control. Every toggle, both directions, now writes a `ModerationAction` (admin id, previous value, new value, timestamp); the `User` row keeps holding only current state by design, `ModerationAction` is the append-only history. Checking whether the invite-token admin actions (revoke, arm, source reassignment) had the same gap turned up: they did. Since the fix was one `logModerationAction` call each, identical in shape to the `ageVerified` fix, all three were included in the same commit rather than deferred.

**Two same-day follow-ups closed items each gap had explicitly deferred.** Adding a Remove action to the reports queue for comment/message reports (Gap 2's deferred item) treats reviewing a report and removing its content as one action — the same shape as the existing `preserve` path for posts, and it reuses (doesn't replace) Gap 1's standalone `DELETE` routes. Unlike `preserve` (gated to red-line post reports only), Remove shows for every comment/message report regardless of category — there's no other legitimate "reviewed, not destroyed" choice for live conversation the way there is for a post. Making `RoomChat` tombstones go live via Realtime (Gap 1's deferred limitation) added a second `postgres_changes` subscription for `UPDATE` on the same table/room filter, sharing one `refetchMessages()` handler with the existing `INSERT` listener — no new RLS policy needed, since Realtime's authorization check is "can this role `SELECT` this row," not operation-specific. Max explicitly approved this one as member-facing UI before it shipped. It could not be verified end-to-end live in this sandbox (no way to hold two authenticated browser sessions against `localhost` — Supabase Auth's redirect allow-list only covers the production domain, the same limitation hit repeatedly elsewhere this session); verified instead via `tsc`/build and a check of Realtime's documented authorization model, with a manual two-tab check recommended before fully relying on it.

Every gap and follow-up was verified against real writes with `test-`-prefixed data, cleaned up after each. `tsc`/build clean throughout; the one schema change (Gap 2's new `ReportTargetType` values) needed no new RLS work — additive enum values on an already-covered table.

### 2026-09-09 — Invitation Panel flow, A5-A7: Hold, real token issuance, email as the only door — plus the copy pass that landed alongside it

Completed the back half of the invitation flow in one continuous push: three-action review, then token issuance, then email delivery, then the final copy pass that made all of it real.

**A5 added Hold as a third, non-terminal review outcome, and the queue itself merged pending and held into one list rather than splitting them.** Requires one of four existing `ApplicationHoldReason` presets plus an optional note; `reviewedAt`/`reviewedBy` double as "who/when last touched this," same as they already did for approve/decline. The status guard was widened to accept "pending" **or** "held" as startable states specifically so a held application stays actionable rather than becoming a dead end. Approve/Decline themselves were deliberately left on the legacy manual-copy-link path in this step — real token issuance was A6, next.

**A6 replaced the legacy manual-copy-link path with real `InviteToken` issuance, and made a point of never exposing the token to the admin.** Accept now mints a token (`source: "application"`) via a new, distinct `Waitlist.applicationTokenId` — the legacy hex field stays frozen, per the A1 decision. Redemption needed zero changes to `/join/[token]`: `"application"` simply falls through the same source-specific conditionals `"purchase_card"` already did (no REP award, no `Referral` row), matching that precedent rather than the older `/invite/[token]` flow's behavior. TTL is `HARD_CAP_DAYS` (see the same-day TTL commit below) from the moment of issuance, with `clientWindowDays` set equal to it so the flat hard cap always governs and the 7-day scan-window mechanic never actually binds for this source — "no scan window" without special-casing the shared arming logic every source uses. The admin-facing copy-link UI was retired outright, replaced with a bare "Approved." confirmation — no `inviteUrl` ever appears in any response from this point on. Verified live end-to-end against `test-`-prefixed data: minted a token, confirmed the legacy field stayed null, then actually redeemed it through real `/join/[token]` — 200, account created, correct (lack of) attribution, legal consent recorded, ritual started empty like every other source.

**The same day, `HARD_CAP_DAYS` dropped from 90 to 20** — a real behavior change, not copy, made in `lib/invites/lifecycle.ts`'s single source-of-truth constant specifically so A6's not-yet-built token issuance would inherit the right value the moment it landed. No currently-mintable token path was affected by the change itself: the print generator that used to read this constant was already retired (A1), and member/partner invites never set `validUntil` at all.

**A7 made email the only door, and treated a failed send as a standing, persistent fact rather than a same-session notification.** Accept sends the already-written, already-approved `sendApplicationAcceptedEmail`; Decline sends `sendApplicationDeclinedEmail` with no reason, per the standing "declines carry no explanation" rule. Both record success/failure onto `Waitlist.decisionEmailSentAt`/`decisionEmailSendError`. Added a `resend` action (checked ahead of the general pending/held guard, since its precondition is "already approved") that reuses the existing token and never mints a new one. Critically, the admin queue's own query was changed to keep surfacing any already-approved application whose email failed — not just for the admin who was watching when it happened — rendered as a loud, persistent red banner with Resend, since a failed send means a real person cannot get in at all. This sandbox has no real `RESEND_API_KEY`, which usefully exercised the failure path end-to-end rather than the success path during verification.

**The copy pass landed the same day, alongside the flow becoming real.** Dropped the application receipt email entirely (the on-screen confirmation carries that job alone now; `receiptEmailSentAt`/`receiptEmailSendError` stay in the schema unused, not migrated away). Confirmation screen reduced to one line: "Your request has been received." Added the two required safety checkboxes to `/invitation`, replacing the earlier placeholder block — flagged rather than silently resolved that the new first checkbox's age clause duplicated the pre-existing standalone 18+ checkbox above it, since the instruction was implemented literally (replacing only the placeholder block) rather than assumed to also cover a merge. That overlap was resolved the same day once asked: the standalone checkbox was removed, and the remaining one carries `ageConfirmed` on its own — confirmed exactly 2 checkboxes render where there had been 3, submission behavior unchanged.

Every piece verified against real `test-`-prefixed data, cleaned up after; `tsc`/build clean throughout; no schema changes beyond what A1 had already staged weeks earlier.

### 2026-09-09 — Admin Console, Part B kickoff: fold four separate admin pages into one console, Step 1 shell

Part B of the two-part invitation-flow task: rebuild `/admin` as a single-screen operational console, replacing `/admin/applications`, `/admin/members`, `/admin/reports`, and `/admin/waiting-list` as separate destinations.

**Folded the four pages into redirect stubs rather than deleting them, so a bookmarked or linked URL still lands somewhere real.** Each now does nothing but `redirect("/admin")`. `/admin/invite-batches` and `/admin/rep` were left untouched — neither was named in the fold instruction, and `invite-batches`' function would later be superseded by Zone 4, but wasn't redirected until that zone actually landed (two days later).

**Built the shell around one piece of state deciding whether the detail panel shows, not a router.** Zones switch via plain component state — never a navigation, so nothing in the console ever triggers a page load. `openId` is that one piece of state, keyed by whichever zone was active when it was opened; switching zones with a panel open just closes it, since there's no cross-zone panel identity to preserve. A single Escape listener lives at the console level, not per-zone, so it always works regardless of which zone's content is on screen. Each zone's list, at this step, was intentionally minimal and real (not fake) data — full per-zone depth was explicitly deferred to the steps that followed, in a specified order.

### 2026-09-10 — Two production interruptions mid-build, both investigate-only, neither a real bug

Two separate reports interrupted Part B within the same day, both resolved as report-only with no code change.

**"Production `/admin` returns 404" turned out to mean nothing had been pushed yet.** Confirmed Part B Step 1 was still local-only (uncommitted at the time), and that the 404 being seen in production was simply the pre-existing `notFound()` gate correctly firing for a non-admin session against the *old*, still-deployed `/admin` page — not a regression.

**"Signed in as `rtmaksim15@gmail.com`, `/admin` still 404s" turned out to mean that account is real, correctly resolving, and simply not an admin.** Investigated `requireAdmin()`'s exact check and both admin accounts' stored state directly against production data: `rtmaksim15@gmail.com` is a genuine, correctly-resolving member account (`isAdmin: false`) created through the normal Waitlist → approve → redeem path, with exactly one matching Supabase Auth identity (no Google-OAuth-creates-a-second-row mismatch). Exactly one row in the entire `users` table has `isAdmin: true` — `lord.obsidian.oc@gmail.com`. No code was changed; the open question ("should `rtmaksim15@gmail.com` become an admin too?") was raised and later explicitly declined — see the founder-exception entry below, where the answer was "no, that's deliberate."

### 2026-09-10 — The Newcomers-room 30-day lockout: a real, confirmed design flaw, fixed at its root

A third interruption the same day, this one a genuine production bug: the founder's own admin account could not complete the Initiation Ritual, because its one remaining step (post once in the Newcomers room) requires a room that a flat 30-day window permanently closes — with no relationship between the two.

**Confirmed the mechanism precisely before proposing any fix.** `lib/rating/room-access.ts#canAccessRoom()`'s `NEWCOMER_WINDOW_DAYS = 30` check is independent of ritual state; `lib/auth/ritual.ts#getRitualStatus()`'s step 4 can only ever be satisfied by an actual message in that same room. Once the window closes without a post, step 4 becomes permanently unsatisfiable — a closed loop with no escape hatch. Queried production directly (read-only) before touching anything: of 2 active members, exactly 1 (`lord.obsidian.oc@gmail.com`, 66 days since joining, never posted) was permanently stuck; 0 others were even mid-window. Also established, and reported, a separate nuance: `/admin` itself carries no ritual gate at all — the practical "can't reach `/admin`" experience was actually the *default post-login redirect to `/feed`* (which is ritual-gated) intercepting first, a distinct, code-free workaround available immediately (navigate straight to `/admin`) while the real fix was still being discussed.

**Implemented the fix exactly as proposed and confirmed: the room stays open past 30 days specifically while the caller's ritual is incomplete, and reverts to the normal rule the moment it completes.** `canAccessRoom()` for the newcomers room now allows access if within the 30-day window **or** the caller's ritual is still incomplete — checked only once the window has actually closed, so the common case (most members, most of the time) stays a single date comparison with no extra query. A new `isRitualComplete()` (`lib/auth/ritual.ts`) mirrors `getRitualStatus()`'s completion definition without its REP-award side effect, since this is access-control code, not a ritual page. This isn't circular: step 4 is exactly the thing keeping the ritual incomplete for someone in this state, so the room staying open lets them post, which clears step 4, which lets the ritual complete, which is what makes the 30-day rule start applying to them again. Verified against real production data: `lord.obsidian.oc@gmail.com` (ritual incomplete) now resolves to access `true`; `rtmaksim15@gmail.com` (already ritual-complete, also past 30 days) correctly still resolves to `false` — confirming the reversion behaves as intended, not just the escape hatch. No production data was touched — the fix was deployed, and the founder completed the ritual normally afterward, exactly as asked.

### 2026-09-10 — Admin Console Step 2: a six-number status bar, with one number as a deliberate alarm state

**Every number is a live count and a filter, not just a stat.** New Applications, On Hold, Failed Sends, Open Reports, Members, Not Age-Verified — clicking any of them jumps to its zone and narrows the list to exactly that subset; a "Showing: X ×" chip clears it. Deliberately *only* these six, on instruction — no doors-state or countdown tie-in; this console is for handling people, not watching the launch.

**Failed Sends gets the club's own alert treatment above zero, not the same quiet styling as the other five** — solid accent fill plus a pulse, since a failed decision-email send means a real person is stuck outside with no way in. Because a failed send can land on an application that's already been decided (the Accept/Decline itself succeeded, only the email didn't), the applications query backing this zone was changed from a plain status filter to an `OR` — "in the pending/held queue" **or** "has a send error" — specifically so a failed send is never invisible just because the underlying decision has already moved on. Also recovered, while touching this query, that the reports list's ordering had quietly regressed to a bare `createdAt` sort when Step 1 first stood the zone up as a minimal list; restored red-line-categories-first, oldest-within-group, matching the pre-fold admin page's own original ordering (see the Zone 3 entry below for where that ordering actually mattered again).

### 2026-09-10 — Launch date moved October 1 → November 1, 2026; a live countdown added to the landing page

**`DOORS_OPEN_DATE` is a runtime environment variable, not a value stored in the repo — updating it in Vercel's dashboard is the actual switch, and no code change alone moves the launch date.** Updated every in-repo trace that *could* be updated (the `.env.example` template and comment, code comments in `lib/config/doors.ts` and the antechamber page) and reported, rather than silently skipped, every place that still says October 1 and was deliberately left alone: `CHANGELOG.md`, this file's own earlier entry, and every attorney-reviewed legal document's effective date — none of those are mine to edit on inference.

**The countdown targets the fixed instant, not local midnight, and needed no server-side date logic of its own.** `2026-11-01T00:00:00-04:00` (midnight America/New_York on that date is still EDT — DST doesn't fall back until 2am that day) parses to one fixed UTC instant regardless of the viewer's own timezone. The component starts rendering only after mount specifically to avoid a hydration mismatch against how much time elapses between server-render and browser-hydration, rather than fighting that mismatch with a suppression flag. Self-clears its own interval and swaps to a placeholder component the instant it reaches zero — verified live, including that it disappears cleanly with no reload and produces no console errors either time.

**The antechamber page needed zero code changes to retarget** — it already derives its "Hall opens ___" copy live from the same `getDoorsState()`/`formatDoorsDate()` the countdown reads, so it retargets automatically the moment the environment variable actually changes.

### 2026-09-11 — Sign-out unreachable from every `/ritual` page: a second production incident, found the same day as the founder-exception request

A member whose ritual is incomplete can reach nothing else in the app — every other gated page bounces back to `/ritual` — and none of `/ritual`'s four pages rendered a sign-out control. `BottomNav` is mobile-only and even there only links to pages that are themselves ritual-gated; desktop carries no navigation chrome outside what a page renders for itself. Net effect: once stuck, there was no way out of the platform by any route, on any device — confirmed live and reported as blocking production for the one account actually affected.

**Fixed at the route-segment level, not per-page, and reused an already-correct sign-out implementation rather than writing a new one.** A new `app/(platform)/ritual/layout.tsx` wraps all four `/ritual*` pages with one persistent, fixed-position Sign Out control — the exact same `SignOutButton` → `POST /api/auth/sign-out` that already worked correctly on `/hall` and `/antechamber` (a real server-side `signOut()` with genuine cookie clearing; nothing was wrong with the mechanism itself, only its reach). This is the same gap `/antechamber`'s own code comment had already flagged and fixed for itself back on 2026-08-08 — it had simply never been carried over to `/ritual`. Verified placement via an unauthenticated fixture, since the real pages sit behind auth this sandbox can't hold a session against on `localhost`.

### 2026-09-11 — Founder exception: Lord Obsidian doesn't pass through the Initiation Ritual at all

Direct production decision, made once the Newcomers-room fix surfaced a second, related fact: the founder's own admin account is also missing its profile step (no bio on file) — meaning even a fixed room wouldn't fully unblock it. Rather than backfill data or grant a one-off exception, the decision was that the founder account is exempt from initiation entirely, on the principle that he's already inside.

**Scoped as one narrow `isAdmin` check at each existing gate, not a general bypass or any change to the ritual logic non-admins go through.** `/feed`, `/compose`, `/members`, and `/hall` each skip the ritual-completion check entirely when `user.isAdmin`; `/ritual` itself redirects an admin straight to `/feed` before computing anything, so the screen never renders for that account at all. `canAccessRoom()` now short-circuits to `true` for `isAdmin` before any other check, including `minLevel` — "already inside" means every room, not just the one this whole incident started with. The doors/antechamber gate needed no change: all four pages already read `!user.isAdmin && getDoorsState().active`, confirmed by grep rather than assumed, so admins were already bypassing that gate before this decision. Verified against real production data: `canAccessRoom` now resolves unconditionally `true` for `lord.obsidian.oc@gmail.com`, and every gate site correctly skips the ritual check for that account, with the ritual logic itself completely untouched for every other member — step 4 keeps working exactly as it did for everyone else.

### 2026-09-10/11 — Admin Console, Zones 1 and 2: Applications and People at full depth

Built the first two of four zones to full depth, each following the same research-first pattern: read the existing (often orphaned, unrouted since the Step 1 fold) queue component and its action route in full before writing anything new, and reuse the route unchanged.

**Zone 1 (Applications) found that "consent history" and "decision history," as named in the original task, aren't real schema for this data — and by instruction, showed only what's actually real rather than inventing a model to match the aspirational language.** There is no consent record for anyone who hasn't yet redeemed an invite (`LegalConsent` requires a real `User` row to exist), and `reviewedAt`/`reviewedBy`/`heldReason`/`heldNote` are last-value-wins columns, not an append-only log. `ApplicationDetail.tsx` ports `ApplicationsQueue.tsx`'s Accept/Hold/Decline/Resend logic — same API contract, same route, untouched — into a single-row detail-panel shape, showing current reviewer, current hold state, decision-email status, and the admin's own age-verified attestation, with no history framing where none exists.

**Zone 2 (People) found the opposite: the "attribution" audit-log model it needs already exists, and one of its two history sections is genuinely real, unlike Zone 1's.** `ModerationAction` (written via `logModerationAction()` since the 2026-09-08 Gap 4 work) is the existing attribution mechanism — no new model needed, same conclusion Zone 1 reached about consent. `LegalConsent`, unlike `Waitlist`'s fields, genuinely is append-only history for a real `User` row (confirmed by its own schema comment and by its write path being a bare `.create()`, never an `.update()`), so this zone shows real consent history, not current-state-only. `RepHistory` is shown here unconditionally, deliberately ignoring `REP_UI_ENABLED` — that flag gates a v1 *member-facing* display decision, not access control, and doesn't apply to an admin surface. Reused the existing `PATCH /api/admin/members/[id]` route unchanged for the `ageVerified` toggle. No ban/promote/role/`inviteAllowance` actions were added — none of those endpoints exist yet, and none were asked for.

Both zones' new queries (post/comment `groupBy` counts, bulk `RepHistory`/`LegalConsent`/`ModerationAction` fetches, the reverse `invitedById` lookup for "invited whom") follow the same "one query per concern across every listed row, not one query per row" shape the Step 2 status bar's counts already established. Verified against real production data where possible (real `RepHistory`/`LegalConsent` rows, confirming the append-only claim directly) and via a temporary unauthenticated fixture for full interaction coverage — the recurring sandbox limitation (no way to hold a real admin session against `localhost`) applied to every step of Part B from here on and isn't re-noted per entry below.

### 2026-09-11 — Admin Console, Zones 3 and 4: Arbitration and Invitations, including a real schema change for direct-by-email invites

**Zone 3 (Arbitration) resolved a genuinely ambiguous spec phrase — "full history" — into three separate, independently real things, rather than guessing at one meaning.** `Report.targetId` is a bare polymorphic uuid with no Prisma relation (the same shape `Waitlist.reviewedBy` already had), so reported content is resolved per `targetType`, recovering the exact per-type lookup pattern the pre-fold `admin/reports/page.tsx` had used (found via git history) and extending it with enough context to build a "view in context" link where the target's own visibility rules make that reliable — notably, `Post` has no admin bypass on its own detail route and no soft-delete field, so once a red-line report is preserved, that link stops working for the admin exactly as it would for anyone, and the panel carries its own copy of the content instead of depending on it. "Full history" became: other reports (any status) against the exact same target, every `ModerationAction` ever logged against that target (not just this report's own resolution — the action route already writes these against the report's real target, never `"report"` itself), and the reporter's own filing stats. All three are shown, labeled separately, rather than picking one. Kept the confirm-dialog-before-action pattern every other zone already used — "one-click" was read as direct context links, not as skipping confirmation, since nothing in the existing UI supported the latter reading.

**Zone 4 (Invitations) needed a real production schema change to satisfy a direct requirement given mid-build: an admin issuing an invitation straight to an email address, bypassing the applications queue entirely.** Added `InviteSource.personal_invitation` and applied it via `prisma db push` against production (this project has no migration files, per its own convention) — using credentials from `.env.local` only, never written into the tracked `.env`, which holds committed placeholders on purpose. `check:rls` re-run clean after, on the reasoning (confirmed by reading `check-rls.ts` itself first) that an added enum value is not a new table and can't trip a table-level check. `/api/join/[token]/route.ts` needed zero changes — every source-specific branch there is a positive `=== "member"`/`=== "partner"` check, so the new value already falls through exactly like `purchase_card`/`application` do today, confirmed by reading the route in full before touching the schema rather than assumed. Needed a new email function (`sendPersonalInvitationEmail`) since neither existing one fit: `sendApplicationAcceptedEmail`'s "your request was accepted" framing is factually wrong with no prior request, and `sendInvitationEmail` (the one with correct non-acceptance framing) omits the expiry line this token's real `validUntil` needs. Send status for this path, and for resending any token with an email on file, is tracked on `InviteToken`'s own `sentToEmail`/`sentToName`/`emailSentAt`/`emailSendError` columns — pre-existing but labeled "batches only" in their schema comment, not actually restricted to that at the database level, and the architecturally correct place regardless since a personally-issued invite has no `Waitlist` row to hold Zone 1's equivalent field. `issued`/`redeemed`/`expired` bucketing is computed live via the existing `evaluateTokenLifecycle()`, not read off the `status` column, since nothing in this codebase has ever written `InviteTokenStatus.expired` — confirmed by grep before relying on the column.

### 2026-09-11 — Admin Console complete: Notes model + RLS, keyboard shortcuts, a mobile pass that found one real bug

**Notes was the one new model the original task had pre-approved, and it was kept to exactly that scope.** `AdminNote` is append-only, same shape as `RepHistory`/`LegalConsent`/`ModerationAction` — a running log of dated observations, not an editable field — and scoped to Zone 2 (People) specifically, where "notes" was actually named in the spec, rather than built as a generic polymorphic table nothing else asked for. Went through the same two-step dance every prior table addition in this codebase has: `prisma db push` created the table with RLS disabled by default, `check:rls` caught it exactly as expected, `alter table admin_notes enable row level security` was run directly against production, `check:rls` re-run clean, and the SQL was saved to `supabase/migrations/` as a record — deny-all, matching every other admin-only table, since this is read and written exclusively via Prisma.

**Keyboard shortcuts were scoped down from the original one-line spec after an explicit check, rather than guessed at.** J/K/A/H/D/Escape/Cmd+K, as named, only fully make sense in the Applications zone (Approve/Hold/Decline) — the other three zones have entirely different action sets. Confirmed the narrower reading before building: J/K move a cursor through whichever zone's list is visible and open that row directly; A/H/D only fire in Applications, only on a still-decidable row, routed through the exact same functions (confirm dialogs included) the click handlers already used, via a ref rather than a second copy of the logic; Cmd+K opens a small "jump to zone" palette, not a full-text search across mixed entity types. Found and fixed a real race live while verifying: the palette's own input originally also handled Escape itself, and in practice that handler won the race against the console's global handler for the same keypress, leaving the global handler reading a stale "is the palette open" value and falling through to closing the underlying detail panel too — collapsing two intended presses into one. Fixed by making the global handler the only place that handles Escape.

**The mobile pass found one real, reproducible overflow bug, not just visual polish.** Two detail-panel grids (`Person` REP/Level/Trust, three columns; `Report`'s Reporter/Reported, two columns) didn't collapse at narrow widths. The two-column one just wrapped awkwardly; the three-column one was worse — the unbreakable "Trust Score" label forced its grid track wider than its container, which pushed the entire detail panel past the viewport edge, confirmed via `document.documentElement.scrollWidth` exceeding `window.innerWidth`, with the panel's own Close control visibly clipped off-screen as the symptom. Both now collapse to one column below Tailwind's existing `sm` breakpoint — confirmed this correctly keys off the *viewport* width, not the panel's own narrower max-width, so desktop needed no change.

This completes Part B: all four zones at full depth, the status bar, Notes, keyboard shortcuts, and a verified mobile pass.

### 2026-09-17 — Landing email capture replaces `WaitingListForm`/`WaitingListEntry` entirely

Supersedes the 2026-08-23 entry above: the landing page's "Request Consideration" secondary path (a bare email field writing to `WaitingListEntry`) is retired in favor of a dedicated, narrower capture with exact required copy ("Invitations arrive with the object. / They cannot be requested. / Leave an address. It may be remembered.").

**A new, dedicated `EmailCapture` table, not a rename of `WaitingListEntry`** — deliberately not reusing the old model, since the two paths' semantics never fully matched (this one never sends email, never mints a token, and is explicitly never contacted). `POST /api/email-capture`: honeypot field, per-IP rate limit, and a fixed minimum response time so new/duplicate/invalid input are indistinguishable by wall-clock timing, all returning the identical `{ ok: true }` — an anti-enumeration posture stricter than `WaitingListEntry` ever had. `hashedIp` (via `lib/security/hash-ip.ts`, salted with `IP_HASH_SALT`) is stored instead of a raw IP, on the same reasoning `AdminAuthEvent` later got for its own `ip` column but applied earlier here since this endpoint is reachable by anyone, logged in or not. Shown in the admin console as a plain read-only count + newest-first list, no actions — matching `WaitingListEntry`'s own original console treatment.

**Retired `WaitingListForm`/`POST /api/waiting-list` in place, same "410, not deleted" pattern the 2026-08-23 entry itself used on the form before it.** `WaitingListForm.tsx` deleted outright only after confirming zero remaining imports; the route itself now unconditionally 410s, matching `POST /api/waitlist`'s own precedent. Deny-all RLS added on `email_captures` and re-confirmed clean.

### 2026-09-17 — Admin identity moves to an env-var allowlist (`ADMIN_USER_IDS`), with mandatory TOTP MFA

Supersedes the 2026-07-02 entry above's authorization semantics: `User.isAdmin` remains a real column (still readable, e.g. for display), but is never trusted for authorization again from this point on — a raw database write to that column can no longer grant admin to anyone.

**`ADMIN_USER_IDS` (Vercel env var, comma-separated `User.id` UUIDs) is the only source of admin identity now.** `lib/auth/admin-allowlist.ts#isAdminId()` parses it once; `getCurrentUser()` (`lib/auth/session.ts`) overwrites `user.isAdmin` with `isAdminId(user.id)` immediately after the Prisma fetch — one choke point, so all ~17 existing `.isAdmin` call sites across the app inherit the hardened value with zero per-site changes.

**`/admin` and every admin API route now require aal2 (a verified TOTP factor), not just allowlist membership.** `requireAdmin()` (`lib/auth/require-admin.ts`) checks both `isAdminId()` and `supabase.auth.mfa.getAuthenticatorAssuranceLevel() === "aal2"`; failing either gives the same 404 a non-member would see, not a 403 — an admin account without MFA enrolled is indistinguishable from outside. `/account/security` (self-service enrollment) is deliberately reachable at aal1, checking `user.isAdmin` directly rather than `requireAdmin()` — an admin with no factor yet has nothing to step up from, and gating enrollment behind the very thing enrollment exists to satisfy would be a lockout with no key. Every sign-in and MFA attempt, success and failure, is logged to a new `AdminAuthEvent` table (deny-all RLS). Rate-limited: 10 password attempts/15min, 5 code attempts/15min, per IP.

**Verified end-to-end against production with a from-scratch RFC 6238 TOTP implementation** (base32 decode + HMAC-SHA1 + dynamic truncation, written to generate valid 6-digit codes from a real enrollment secret without a physical phone) — real enroll → wrong-code-rejected → real-code-succeeds → aal2-achieved, and `requireAdmin()`-equivalent pass/fail/fail scenarios, against two disposable accounts (one with a DB `isAdmin: true` flag specifically to prove it's ignored). Documented for the founder in `docs/ADMIN-ACCESS.ru.md`, leading with the fact that `/admin` opens for no one, including him, until `ADMIN_USER_IDS` is actually set in Vercel Production — a more basic prerequisite than MFA itself.

### 2026-09-18 — Launch preview: `PREVIEW_USER_IDS` bypasses only the holding page

A small, manually-invited set of accounts can experience the platform as a launched member would, while every other real member still sees the pre-launch holding page (`/antechamber`) until `DOORS_OPEN_DATE`.

**`PREVIEW_USER_IDS` (same shape as `ADMIN_USER_IDS`: comma-separated `User.id` UUIDs) is deliberately not the same mechanism as `isAdmin`.** A preview account still has to pass the Initiation Ritual and every room-access rule (level, the Newcomers 30-day window) — only the doors/antechamber gate itself is skipped. `lib/config/doors.ts#bypassesDoors(user)` is the single choke point (`user.isAdmin || isPreviewId(user.id)` at the time), wired into every gate site that used to check `!user.isAdmin` alone: `feed`, `compose`, `members`, `hall`, `messages` and its two subpages, and the nav-visibility (`fullAccess`) computation in `(platform)/layout.tsx`. The three `messages/*` pages had ritual and doors bundled under one `if (!user.isAdmin)`; split apart so a preview account still gets stopped by the ritual check while only the doors check goes through `bypassesDoors()`.

**Messages became reachable from the UI in the same pass** — a "Messages" item in `DesktopNav` (between Profile and Sign out) and a sixth tab in `BottomNav`, both with an unread dot (`ThreadParticipant.lastReadAt`, a new nullable column, set on each thread-page view; unread = a pending request, or a thread's latest message newer than the participant's own `lastReadAt`). Verified live via real HTTP against production between two disposable accounts: request → accept → send → receive-live (Realtime) → report → block, and separately confirmed the bypass itself (a preview-flagged account reaches `/feed`/`/messages` while doors are active; a plain account is still sent to `/antechamber`) using a local server against the same production database, since `PREVIEW_USER_IDS` wasn't yet set in Vercel at verification time.

### 2026-09-21 — Normal-operation mode (`APP_OPEN`) and the Newcomers-window floor

Builds on, rather than replaces, the 2026-09-10 "Newcomers-room 30-day lockout" entry above — that fix (the room stays open past 30 days while the ritual is incomplete) is untouched; this adds a second, independent reason the window itself can be longer than 30 raw days from `joinedAt`.

**`APP_OPEN` (env var, `"true"`/anything else) turns off the holding-page gate for every signed-in member at once, not just an allowlisted few.** Folded into the same `bypassesDoors()` choke point the `PREVIEW_USER_IDS` entry above established: `isAppOpen() || user.isAdmin || isPreviewId(user.id)` — no per-page changes needed, and `PREVIEW_USER_IDS` keeps working, just redundant once `APP_OPEN=true`. Ritual, legal reconsent, and DM-rules acceptance are separate gates, entirely untouched. Deliberately independent of `getDoorsState()`/`DOORS_OPEN_DATE`, which the public landing page's countdown reads directly — `APP_OPEN` only feeds into `bypassesDoors()`, never `getDoorsState()` itself, so the countdown is unaffected by design, not by accident.

**The Newcomers window now starts from the later of `joinedAt` and "the moment the app opened for them"** — a member invited before that moment shouldn't lose days they could never have used. That moment is `getAppOpenedAt()` (a new `AppState` table, a single permanent row, `appOpenedAt` set exactly once — the first time any request observes `APP_OPEN=true`, since an env var carries no timestamp of its own) while `APP_OPEN=true`, else `DOORS_OPEN_DATE`. On top of the floor, while `APP_OPEN=true` and "now" is still before `DOORS_OPEN_DATE` (or it's unset) — the manually-invited soft-launch phase, before the real announced date — Newcomers is open to every member outright, floor math aside: a small hand-picked early cohort shouldn't be able to lock each other out of the one room that's also how the ritual's "introduce yourself" step gets finished.

**Verified locally against the production database** (a temporary local-only `APP_OPEN=true`, since setting the real Vercel var is the founder's own action): a freshly-created, ritual-complete member reaches `feed`/`hall`/`rooms`/`messages`; a member backdated 40 days still reaches Newcomers, confirmed two ways — once under the blanket pre-launch rule, and again with `DOORS_OPEN_DATE` forced into the past specifically to prove the floor math itself (not just the blanket rule) works. The test-created `AppState` row was deleted afterward so the real first production activation gets its own accurate timestamp, not a QA one.
